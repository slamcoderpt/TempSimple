...
export default function TablePropertiesPanel({ show, onClose, fields, onSave, onBack, project }) {
    ...
    const [localFields, setLocalFields] = useState(fields?.map(field => ({
        ...field,
        visible: field.is_visible ?? !field.hidden,
    })) || []);

    useEffect(() => {
        setLocalFields(fields?.map(field => ({
            ...field,
            visible: field.is_visible ?? !field.hidden,
        })) || []);
    }, [fields]);
                                        ...
                                        <div className="border-t border-gray-200 p-4 space-y-4">
                                            <div className="flex items-center justify-between">
                                                <span className="flex items-center gap-2 text-sm text-gray-500">
                                                    ...
                                                    Hidden properties ({localFields.filter(f => !f.visible).length})
                                                </span>
                                            </div>
                                            <button
